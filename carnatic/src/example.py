...
def _get_note_frequency_duration(note_array):
    ...
    for i,note in enumerate(note_array):
        if (re.match(_NOTES_PATTERN_1,note)):
            ...
            durn = cplayer._FULL_NOTE_DURATION / ( ( 2 ** (speed-1) ) *1.0 ) 
            inst = cplayer.DEFAULT_INSTRUMENT
            ...
            print("Last note",result[-1])
            result[-1][1][-1] += cplayer._FULL_NOTE_DURATION
        elif note.strip() == ";":
            print("Last note",result[-1])
            result[-1][1][-1] += 2 * cplayer._FULL_NOTE_DURATION
